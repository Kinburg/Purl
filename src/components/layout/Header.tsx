...
function HeaderImpl() {
  // Selectors instead of destructuring — Header re-renders on every project
  // change. Most fields are accessed only inside event handlers; pulling them
  // via selectors keeps Header subscribed to exactly what it reads.
  ...
  // File-system workflows + their UI state live in a dedicated hook.
  const {
  ...
  } = useProjectFileOps({ closeFile: fileDD.close, closeExport: exportDD.close, closeSc: scDD.close });
